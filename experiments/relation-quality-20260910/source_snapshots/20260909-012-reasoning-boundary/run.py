...
def run_one(job, repeat, max_context):
    ...
    try:
        ...
        if job['mode'] != 'off' and not counts.get('think_end_marker_before_final'):
            raise ValueError('Enabled thinking region did not close before the final answer')
...
def main():
    ...
    # Public, non-material probes qualify both boundary states, not semantic scores.
    for mode in ['low', 'off']:
        probe_path = OUT / f'compatibility-probe-{mode}.json'
        if probe_path.exists():
            if not json.loads(probe_path.read_text())['passed']:
                raise RuntimeError('Previous compatibility probe failed; no retry-to-win')
            continue
        schema = {'type': 'object', 'additionalProperties': False, 'properties': {'value': {'type': 'integer'}}, 'required': ['value']}
        final_tag = {'type': 'tag', 'begin': BEGIN, 'content': {'type': 'json_schema', 'json_schema': schema}, 'end': END}
        fmt = final_tag if mode == 'off' else {'type': 'sequence', 'elements': [
            {'type': 'tag', 'begin': '', 'content': {'type': 'any_text', 'excludes': [BEGIN]}, 'end': '</think>'},
            {'type': 'regex', 'pattern': '[ \\t\\r\\n]*'}, final_tag]}
        body = {'model': model['id'], 'messages': [{'role': 'user', 'content':
                'Compute (13*17+29)/5. Return the result as <final_json>{"value":integer}</final_json>. '
                'If thinking is enabled, close it with </think> before the final tag; otherwise start the final tag directly.'}],
                'temperature': 0, 'max_tokens': 512, 'skip_special_tokens': False,
                'chat_template_kwargs': {'enable_thinking': False} if mode == 'off' else {'enable_thinking': True, 'reasoning_effort': 'low'},
                'response_format': {'type': 'structural_tag', 'format': fmt}}
        response = call('/v1/chat/completions', body, timeout=180)
        answer_text, counts = final_only(response['choices'][0]['message'].get('content'))
        passed = bool(counts.get('final_region_closed')) and (mode == 'off' or counts.get('think_end_marker_before_final'))
        final = json.loads(answer_text) if passed else None
        if passed:
            jsonschema.validate(final, schema)
        save(probe_path, {'passed': bool(passed), 'counts': counts, 'usage': response.get('usage'), 'final_answer': final})
        response = None
        if not passed:
            raise RuntimeError('THINK_BOUNDARY_PROBE_FAILED')
    preflight = []
