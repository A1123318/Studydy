...
def main():
    ...
    for mode in ['xhigh', 'low', 'off']:
        for repeat in [1]:
            ...
            for label, included in [('PN', ['P', 'N']), ('PNI', ['P', 'N', 'I'])]:
                ...
                answered = sum(groups[g]['answered'] for g in included)
                scores[label] = {'correct': num, 'total': den, 'percent': round(num / den * 100, 2) if complete else None,
                                 'answered': answered, 'answered_only_percent': round(num / answered * 100, 2) if answered else None}
            ...
            summaries.append({'mode': mode, 'repeat': repeat, 'complete': complete,
                'calls': len(calls), 'planned_calls': 4, 'completed_calls': sum(r['status'] == 'completed' for r in calls),
                'completion_percent': round(sum(r['status'] == 'completed' for r in calls) / 4 * 100, 2) if complete else None, 'statuses': dict(Counter(r['status'] for r in calls)), 'groups': groups, **scores,
    ...
    report = {'summaries': summaries, 'vs_xhigh': differences,
        'limits': ['Only one round of60 cases per mode by user request; no evidence of stable advantage across repeats', 'Prefix characters are not exact reasoning-token counts',
