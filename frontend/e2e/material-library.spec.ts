...
test("fresh profiles discover their own materials and reopen both exact published versions", async ({ browser }) => {
  ...
  await expect(page.locator(".learning-path li")).toHaveCount(1);
  await page.reload();
  await expect(page.getByRole("button", { name: "教材概念：Stack", exact: true })).toBeVisible();
  ...
  const pdfUrl = await freshPage.getByRole("link", { name: "開啟原始 PDF", exact: true }).getAttribute("href");
  ...
  await freshPage.getByRole("button", { name: "開啟版本 1", exact: true }).click();
  await expect(freshPage.getByRole("button", { name: "教材概念：Stack", exact: true })).toBeVisible();
  const olderPath = new URL(freshPage.url()).pathname;
