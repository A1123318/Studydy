...
export function MaterialLibrary({ apiClient, materialId, mapsOnly = false }: { apiClient: StudydyApiClient; materialId?: string; mapsOnly?: boolean }) {
  ...
  return <section className="material-library">
    ...
    {visibleItems.length === 0 && <section className="library-empty surface" aria-label="空教材引導">
      ...
      <h2>{mapsOnly ? "尚未建立知識地圖" : "尚未有學習教材"}</h2>
      <p>{mapsOnly ? "上傳 PDF 建立地圖，或到我的教材查看處理狀態。" : "請上傳您的第一份 PDF，讓 Studydy 陪你展開學習。"}</p>
      <button className="primary-button" type="button" onClick={() => writeRoute({ name: "upload" })}><Icon name="upload" size={18} />上傳第一份教材</button>
      {mapsOnly && <button className="text-button" type="button" onClick={() => writeRoute({ name: "materials" })}>前往我的教材</button>}
    </section>}
    ...
    {visibleItems.map(item => {
      ...
      return <article className="surface library-item" key={item.material_id} aria-label={item.display_name}>
        ...
        <div className="state-actions">
          ...
          {available[0] && <button className={item.study_sessions[0] ? "secondary-button" : "primary-button"} type="button" onClick={() => openStructure(item, available[0])}>開啟知識地圖</button>}
